...
contract ERC1820Registry {
    ...
    function getInterfaceImplementer(
        ...
        address addr = _addr == address(0)
            ? msg.sender
            : _addr;
        if (isERC165Interface(_interfaceHash)) {
            bytes4 erc165InterfaceHash = bytes4(
                _interfaceHash
            );
            return
    ...
    function setInterfaceImplementer(
        ...
        if (
            _implementer != address(0) &&
            _implementer != msg.sender
        ) {
            require(
    ...
    {
        require(
            getManager(_addr) == msg.sender,
            "Not the manager"
        );
        managers[_addr] = _newManager == _addr
            ? address(0)
            : _newManager;
